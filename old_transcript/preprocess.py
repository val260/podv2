...
def reduce_noise_centroid_mb(y, sr):
    ...
    less_noise = AudioEffectsChain().lowshelf(
        gain=-30.0, frequency=threshold_l, slope=0.5).highshelf(
            gain=-30.0, frequency=threshold_h, slope=0.5).limiter(gain=10.0)
    ...
    boost_bass = AudioEffectsChain().lowshelf(
        gain=16.0, frequency=boost_h, slope=0.5)#.lowshelf(gain=-20.0, frequency=boost_l, slope=0.8)
...
def reduce_noise_mfcc_up(y, sr):
    ...
    speech_booster = AudioEffectsChain().lowshelf(
        frequency=min_hz*(-1), gain=12.0, slope=0.5)#.highshelf(frequency=min_hz*(-1)*1.2, gain=-12.0, slope=0.5)#.limiter(gain=8.0)
    y_speach_boosted = speech_booster(y)
...
------------------------------------'''
def enhance(y):
    apply_audio_effects = AudioEffectsChain().lowshelf(
        gain=10.0, frequency=260, slope=0.1).reverb(
            reverberance=25, hf_damping=5, room_scale=5, stereo_depth=50, pre_delay=20, wet_gain=0, wet_only=False)#.normalize()
...
def noise_reduce_scipy(data, rate):
    b, a = signal.butter(4, 10000, 'low')
    x_fil = signal.filtfilt(b, a, data)
    return x_fil
