...
def run_transcription(video, output_dir):
    ...
    for entry in ch_rep:
        ...
        size_div = math.ceil(len(data) / (NB_WORKERS_POOL * 4))
        for i in range(NB_WORKERS_POOL*4):
    ...
    p.join()
    list_res.sort()
    list_res = merge_seg(list_res, data, rate, lang)
    #list_res = convert_to_readible(list_res, rate)
    #msg += deepspeech2VTT(video, list_res, folder, output_dir)
    msg += mergeVTT(video, list_res, rate)
    return msg #list_res
...
def run_aux(data, rate, window, overlap_o, overlap_t, start, end):
    ...
    while (sample_end < end - 1):
        ...
        # Compute the maximal right boundary to get all words the sentence
        # during the recognition
        while ((new is None or old != new) and sample_end < end - 1):
            if new:
                old = new
            ...
            sample_window += overlap_o
        if old and old == new:
            sample_end -= overlap_o
        else:
            old = new
        if sample_start < sample_end - 1:
            cond1 = True
        # Compute the maximal left boundary to avoid silence at the begining
        # of recognition
        while (old and old == new and sample_start < sample_end - 1):
            sample_start = sample_start + overlap_t
            if sample_start >= sample_end - 1:
                sample_start = sample_end - 1
            data_window = data[sample_start:sample_end]
            new = ds_model.stt(data_window, rate)
        if cond1 and old != new:
            sample_start -= overlap_t
            new = old
        if sample_end > sample_start + 1:
            cond2 = True
        # Compute the minimal right boundary to avoid silence at the end
        # of recognition
        while (old and old == new and sample_start + 1 < sample_end):
            sample_end = sample_end - overlap_t
            if sample_start + 1 >= sample_end:
                sample_end = sample_start + 1
            data_window = data[sample_start:sample_end]
            new = ds_model.stt(data_window, rate)
        if cond2 and old != new:
            sample_end += overlap_t
        # Store the final result into the result list
        if old and (cond1 or cond2):
            res.append((sample_start, sample_end, old))
        elif new and not (cond1 or cond2):
            res.append((sample_start, sample_end, new))
        sample_start = sample_end
    return res


def merge_seg(seg_list, data, rate, lang):
    res = []
    ...
    for i in range(len(seg_list) - 1):
        ...
        if start and to_merge:
            ...
            new = ds_model.stt(data_window, rate)
            res.append((speech_begin, speech_end, new))
        elif not start:
